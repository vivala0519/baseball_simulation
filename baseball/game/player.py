...
class ChatConsumer(WebsocketConsumer):
    ...
    def inning_proc(self, homePitchers, homeBatters, awayPitchers, awayBatters):  # 경기 진행 로직
        ...
        # 선택된 선수의 기록에 해당년도의 리그 출루율/피출루율 요소를 append한다.
        ...
        innings = range(1, 4)  # 이닝(1 ~ 12)   테스트용은 3회까지
        ...
        for inning in innings:  # 9 ~ 12이닝
            for top_and_bottom in range(1, 3):  # 이닝별 초, 말
                ...
                if top_and_bottom == 1:  # 이닝 초
                    ...
                    # 각 선수의 기록으로 경기를 진행할 수 있도록 해주는 부분. 선수 바뀌면 다시 호출해야한다.
                    ...
                    while True:  # 3아웃 되는 동안 실행
                        ...
                        elif home_pitcher == home_sp and \
                                (home_sp_pitch_count > 100 or home_sp_scored > 3):  # 선발투수 투구 수가 100개 초과이거나 3실점 초과일 경우
                        ...
                        if game.main(home_pitcher[7], away_hitter_num[11]):  # 투수 VS 타자가 아웃일 경우
                            ...
                            # 아래의 (away_hitterIndex+1)는 실제로는 [away_hitterIndex]번째를 가리킨다.
                            ...
                            if home_pitcher == home_sp:  # 투수가 선발투수일 시 투구 수 추가
                                if o_res == '삼진':
                                    ...
                                    self.record_send(str(inning) + '회 : ' + '투구 수 : ' + str(home_sp_pitch_count))
                            elif home_pitcher == home_rp:  # 투수가 중계투수일 시 투구 수 추가
                                if o_res == '삼진':
                                    ...
                                    self.record_send(str(inning) + '회 : ' + '투구 수 : ' + str(home_rp_pitch_count))
                            elif home_pitcher == home_cp:  # 투수가 마무리투수일 시 투구 수 추가
                                if o_res == '삼진':
                    ...
                    out = 0
                    while True:
                        if away_pitcher == away_sp and (away_sp_pitch_count > 100 or away_sp_scored > 3):
                            self.record_send('rp로 투수 교체')
                            away_sp_pitch_count = 0
                            away_sp_scored = 0
                            away_pitcher = away_rp
                        elif away_pitcher == away_rp and (away_rp_pitch_count > 100 or away_rp_scored > 3):
                        ...
                        if game.main(away_pitcher[7], home_hitter_num[11]):
                            ...
                            self.record_send(str(home_hitterIndex+1) + '번 타자, ' + o_res)
