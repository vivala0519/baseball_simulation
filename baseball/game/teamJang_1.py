...
class Game():
    ...
    pitcher_league_OBP = 0.0
    batter_league_OBP = 0.0

    # 한 게임 시작 시, 객체 생성하면서 생성자 파라미터로
    # 해당 리그의 투수 리그 피출루율, 타자 리그 출루율을 넣어준다.
    def __init__(self, pitcher_league_OBP, batter_league_OBP):
        self.pitcher_league_OBP = pitcher_league_OBP
        self.batter_league_OBP = batter_league_OBP
    ...
    #
    #     self.pitcher_name = pitcher

    # def batter_OBP(self):       # 타자 출루율
    #
    #     batter_obp = float('0' + soup.select("span")[225].string)
    #
    #     return batter_obp
    #
    # def pitcher_OBP(self):      # 투수 피출루율
    ...
    def main(self, pitcher_stat, batter_stat):     # 타자 투수 대결 결과 (안타, 아웃 중 출력)

        pitcher = pitcher_stat['onbase']
        batter = batter_stat['onbase']
        batter_oz = batter / (1 - batter)
        pitcher_oz = (1 - pitcher) / pitcher
        pitcher_league_obp = self.pitcher_league_OBP
        batter_league_obp = self.batter_league_OBP
